...
describe('fillZero', () => {
  // '값이 지정된 자릿수보다 작은 경우 앞에 0을 채워 "05"로 반환한다'
  test("5를 2자리로 변환하면 '05'를 반환한다", () => {
    expect(fillZero(5, 2)).toBe('05');
  });

  // '값이 자릿수와 일치하는 경우 그대로 반환하여 "10"을 반환한다'
  test("10을 2자리로 변환하면 '10'을 반환한다", () => {
    expect(fillZero(10, 2)).toBe('10');
  });

  // '0을 채워서 3자리로 변환하면 "003"을 반환한다'
  test("3을 3자리로 변환하면 '003'을 반환한다", () => {
    expect(fillZero(3, 3)).toBe('003');
  });

  // '값이 지정된 자릿수를 넘으면 그대로 반환하여 "100"을 반환한다'
  test("100을 2자리로 변환하면 '100'을 반환한다", () => {
    expect(fillZero(100, 2)).toBe('100');
  });

  // '기본값 2를 사용하여 5를 "05"로 반환한다'
  test('size 파라미터를 생략하면 기본값 2를 사용한다', () => {
    expect(fillZero(5)).toBe('05');
  });

  // '소수점 있는 값 3.14를 5자리로 변환하면 "03.14"를 반환한다'
  test("소수점이 있는 3.14를 5자리로 변환하면 '03.14'를 반환한다", () => {
    expect(fillZero(3.14, 5)).toBe('03.14');
  });

  // 아래는 중복되는 테스트 케이스로 생략해도 될 것 같습니다.
