...
beforeAll(() => {
  vi.useFakeTimers();
  vi.setSystemTime(new Date('2024-10-01T00:00:00.000Z').getTime());
});
...
describe('useCalendarView Hook - 초기 상태 테스트', () => {
  ...
  it('currentDate는 오늘 날짜인 "2024-10-01"이어야 한다', () => {
    const { result } = renderHook(() => useCalendarView());
    const fixedToday = new Date('2024-10-01');
    assertDate(result.current.currentDate, fixedToday);
  });
...
describe('useCalendarView Hook - navigate 함수 테스트', () => {
...
describe('useCalendarView Hook - view 변경 테스트', () => {
...
describe('useCalendarView Hook - 공휴일 업데이트 테스트', () => {
